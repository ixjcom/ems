...
public class PermissionConfig {
	...
	private static final String PERMISSION_PATH = "classpath*:*permission.xml";
	...
	public void init() {
		try {
			...
			throw new RuntimeException("加载权限配置文件时发生异常。", e);
		}
	}
	...
	private void initPermissions() throws IOException {
		...
		for (Resource permissionResource : permissionResources) {
			...
			permissions.merge((Permissions) xstream.fromXML(permissionResource.getInputStream()));
		}
	}
		...
		for (PermissionGroup permissionGroup : getPermissionGroups()) {
			for (Permission permission : permissionGroup.getPermissions()) {
				if (allPermissions.contains(permission)) {
					throw new RuntimeException("权限[" + permission + "]定义重复。");
				}
	...
	public Permission getPermission(String permissionCode) {
		...
		throw new RuntimeException("未找到权限编码为" + permissionCode + "的权限。");
	}
	 ...
	 *
	 * @param newPermissionGroups 新构建的权限分组列表
	 * @param permission          权限
	 */
	...
	private PermissionGroup findGroupByPermission(Permission permissionToFind) {
		...
		throw new RuntimeException("在所有权限分组中未找到权限编码为"+ permissionToFind.getCode() + "的权限。");
	}

	public  List<Resource> getResourcesByWildcard(
